...
# Side Bar Panel
Contains the inputs that will be used in different `sub-panels`. The following five inputs will be used in sub-panels **Basis Functions** (to generate the basis functions) and **MNCSDE** (to initialize, run and report the `MNCSDE` algorithm):
...
The first two inputs are mostly to describe the *basis functions* and *initialized* the *MNCSDE*. The third input is to specify the number common basis in *MNCSDE* algorithm, and number of clusters to produce the outputs. The fourth input specify the type of input (time series or periodograms).
...
## MNCSDE
* The _MNCSDE_ algorithm is implemented in this sub-panel
