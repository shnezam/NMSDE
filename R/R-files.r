...
initial<-function(X, nbasis=10, norder=4, R=length(input$model), pen="diff", a=2, theta_P2=T, tru=NULL, f.r=NULL){
           ...
           P<-nrow(X[[1]]);P2<-P^2; Nreal<-P*(P+1)/2; len<-ncol(X[[1]]); m<-length(X)
           K<-floor((len-1)/2); omegas<-(1:K)/len
           if(is.null(f.r)){f.r<-c(1,length(omegas))
           } else{omegas<-omegas[f.r[1]:f.r[2]]; K<-length(omegas)}
           ...
           if(pen=="diff") {if (a!=0) {L<-diff(diag(nbasis),differences=2)} else{L<-diag(nbasis)};D<-t(L)%*%L
           } else{D<-fda::bsplinepen(fda::create.bspline.basis(nbasis=nbasis,norder=norder))}
           
           init<-list(P=P,m=m,K=K,omegas=omegas,f.r=f.r,B=B,D=D,indexer=indexer,Nreal=Nreal,knots=knots)
           ...
           #get true spectrum
           if(!is.null(tru)){init$truspec<-sapply(tru$paras,psd,tru$sigma,init,simplify = "array");init$rnd<-tru$rnd}
           return(init)
         }
#-----------------------------------------------------------------------------------------------------
## time2freq
...
time2freq<-function(x,init){
             ...
             for(k in 1:init$K){
               ...
               for(p in (init$Nreal+1):init$P^2) perds[p,k]<-Im(P_mat[init$indexer[p,3],init$indexer[p,4]])
             }
...
NWspec<-function(x,init){
          logh<-2*log(20/init$K); h<-exp(logh)
...
#-----------------------------------------------------------------------------------------------------
## inChol2spec
#Takes the inverse Cholesky components and returns the components of the spectral matrix
#' @export inChol2spec
inChol2spec<-function(inChol,init){
               ...
               for(k in 1:init$K){
                 ...
                 for(p in 1:init$Nreal) spec[p,k]<-Re(F_k[init$indexer[p,3],init$indexer[p,4]])
                 for(p in (init$Nreal+1):P2) spec[p,k]<-Im(F_k[init$indexer[p,3],init$indexer[p,4]])
               }
               return(spec)
             }
#-----------------------------------------------------------------------------------------------------
## psd
#True power spectral density(PSD) for VAR(p)
#' @export psd
psd<-function(Phi,sigma,init){
       P2<-init$P^2
       #psd at freq w
       F<-function(w){
...
iterate<-function(init, lambda=0, a=2, eps=0.5, n.iter=100, which.spec=NULL, update.lam=T){
   thetas.lst<-list(); As.lst<-list(); ploglikes<-rep(0,0); AIC<-rep(0,0); Lambda<-list()
   l<-1       
   if(!is.null(which.spec)){
     init_i<-init.theta.A_i(init,i=which.spec)
     init$thetas<-init_i$thetas;init$As<-init_i$As; init$m<-length(which.spec) 
     init$freqdom<-init$freqdom[which.spec]
   }
   thetas.lst[[l]]<-Re(init$thetas); As.lst[[l]]<-Re(init$As); Lambda[[l]]<-lambda
   ...
   while(abs(diff)>eps && l<n.iter){
     ...
     thetas.lst[[l+1]]<-Re(workvals$thetas); As.lst[[l+1]]<-Re(workvals$As)
     ...
     if(update.lam){
      ...
      l <- l+1; #incProgress(1/n.iter, detail = paste("iteration", l)) 
...
plot.sds<-function(res,init){
   ...
   for(i in 1:init$m){
     for(j in 1:init$P^2){
       plot(init$omegas,inChol2spec(tA2G(res$thetas[[length(res$thetas)]],res$As[[length(res$As)]],init)[,,i],init)[j,],"l",xlab="",ylab="",ylim=c(0,5),main=j)
...
drivG<-function(init){
         ...
         for(j1 in 1:init$P^2) for(j2 in 1:init$P^2){
...
tA2G<-function(thetas,As,init){
        ...
        for(j in 1:init$P^2){
          if(!is.matrix(thetas)){theta.tA[,,j]<-thetas[,,j]%*%t(As[,,j])
          } else{theta.tA[,,j]<-thetas%*%t(As[,,j])}
...
Workingfish<-function(thetas=ini$thetas, As=ini$As, lambda, init){
  nbasis<-ncol(init$B); norder<-nbasis-length(init$knots)+2; P2<-init$P^2; R<-ncol(init$thetas)
  if(!is.matrix(thetas)){
    U.theta<-matrix(0,nr=nbasis*P2,nc=R); H.theta<-array(0,dim=c(nbasis*P2,nbasis*P2,R))
    F.theta<-array(0,dim=c(nbasis*P2,nbasis*P2,R)); tmp1i<-F.theta
    theta.chng<-matrix(0,nr=nbasis*P2,nc=R); curr.thetas<-array(0,dim=c(nbasis,R,P2))
    theta_L.P2_R<-apply(thetas,2,as.vector) # L*R*P2 array (thetas) into L.P2*R matrix
  } else{
    U.theta<-matrix(0,nr=nbasis,nc=R); H.theta<-array(0,dim=c(nbasis,nbasis,R))
    F.theta<-array(0,dim=c(nbasis,nbasis,R)); tmp1i<-F.theta 
    theta.chng<-matrix(0,nr=nbasis,nc=R); curr.thetas<-matrix(0,nr=nbasis,nc=R)
  }
  
  A.chng<-matrix(0,nr=R*P2,nc=nrow(As)); curr.As<-array(0,dim=c(nrow(As),R,P2))
  A_R.P2_m<-apply(As,1,as.vector) # m*R*P2 array (As) into R.P2*m matrix
  hatG_m<-tA2G(thetas,As,init); hatG_m_list<-lapply(seq(dim(hatG_m)[3]),function(x)hatG_m[,,x])
  ...
  for(i in 1:nrow(As)){                                              
    ...
    G_i<-matrix(0,nr=init$P,nc=init$P); F_i<-matrix(0,nr=init$P,nc=init$P)
    ...
    for(k in 1:init$K){
      ...
      temp2_k_i<-matrix(0,nr=P2,nc=P2); temp3_k_i<-temp2_k_i ; temp8_k_i<-temp2_k_i
      ...
      for(j1 in 1:P2){
        ...
        for(l in which(init$B[k,]!=0)) for(r in 1:R){temp4_i[l,j1,k,r]<-temp1_i[j1,k]*As[i,r,j1]*init$B[k,l]}
        for(j2 in 1:P2){
          ...
          for(l1 in which(init$B[k,]!=0)) for(l2 in which(init$B[k,]!=0)) for(r in 1:R){
  ...
  while(repit){
     ...
     if(!is.matrix(thetas)){for(r in 1:R) curr.thetas[,r,]<-matrix(theta_L.P2_R[,r]-tau*theta.chng[,r],nr=nbasis,nc=P2)
     } else {for(r in 1:R) curr.thetas[,r]<-thetas[,r]-tau*theta.chng[,r]}
     if(loglike(curr.thetas,curr.As,lambda,init)$ploglike<=loglike(thetas,As,lambda,init)$ploglike) {repit<-FALSE} else {counter<-counter+1}
